...
export function FileList({ files, onDelete, onSelect, loading }: FileListProps) {
  ...
  const getFileIcon = (mimeType: string) => {
    ...
    // Word 文档 (.doc, .docx)
    if (mimeType?.includes('word') || mimeType?.includes('document') || mimeType?.includes('msword') || mimeType?.includes('openxmlformats-officedocument.wordprocessingml')) return (
      <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        <text x="9" y="10" fontSize="6" fill="currentColor" fontWeight="bold">W</text>
      </svg>
    );
    // Excel 文档 (.xls, .xlsx)
    ...
    // PowerPoint 文档 (.ppt, .pptx)
    if (mimeType?.includes('powerpoint') || mimeType?.includes('presentation') || mimeType?.includes('ms-powerpoint')) return (
      <svg className="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 13h6M9 17h4" />
      </svg>
    );
    if (mimeType?.includes('zip') || mimeType?.includes('rar') || mimeType?.includes('7z') || mimeType?.includes('tar') || mimeType?.includes('gzip')) return (
  ...
  if (files.length === 0) {
    return (
      <div className="text-center py-16">
        <div className="w-16 h-16 mx-auto bg-violet-50 dark:bg-violet-900/20 rounded-2xl flex items-center justify-center mb-4">
  ...
  return (
    <div className="space-y-2 max-h-[500px] overflow-y-auto">
      {files.map((file) => (
        <div
          key={file.id}
          className="flex items-center justify-between p-3 bg-white dark:bg-gray-900/50 rounded-xl border-l-2 border-transparent hover:border-violet-400 hover:bg-violet-50/40 dark:hover:bg-violet-900/10 transition-all duration-150 group"
