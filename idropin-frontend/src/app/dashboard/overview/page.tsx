...
function OverviewSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      <div className="mb-8">
        <div className="h-8 skeleton w-40 rounded-lg mb-2" />
        <div className="h-4 skeleton w-24 rounded" />
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className="card p-6">
            <div className="w-10 h-10 skeleton rounded-lg mb-3" />
            <div className="h-3 skeleton w-16 rounded mb-2" />
            <div className="h-7 skeleton w-12 rounded mb-1" />
            <div className="h-3 skeleton w-24 rounded" />
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 card p-6 h-52 skeleton" />
        <div className="card p-6 h-52 skeleton" />
      </div>
    </div>
  );
}
  ...
  const cards = [
    {
      ...
      ib: 'bg-blue-50 dark:bg-blue-900/20',
    },
    {
      title: '占用空间',
    ...
    },
    {
      ...
      ib: 'bg-amber-50 dark:bg-amber-900/20',
    },
  ];
    ...
    <div className="space-y-6 page-enter">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((card, i) => (
          <div
            key={i}
            className="card p-6 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default animate-slide-in-up"
            style={{ animationDelay: `${i * 60}ms` }}
          >
            <div className={`w-10 h-10 rounded-lg ${card.ib} ${card.ic} flex items-center justify-center mb-3`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 card p-6 animate-slide-in-up" style={{ animationDelay: '180ms' }}>
              ...
              <tbody>
                {recentFiles.length > 0 ? (
                  recentFiles.map((file) => (
                    <tr key={file.id}>
                      <td className="max-w-[120px] sm:max-w-[200px] truncate">{file.originalName}</td>
                      <td className="hidden sm:table-cell">{formatFileSize(file.fileSize)}</td>
                      <td className="text-xs sm:text-sm whitespace-nowrap">{new Date(file.createdAt).toLocaleString()}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
        ...
        <div className="card p-6 animate-slide-in-up" style={{ animationDelay: '240ms' }}>
            ...
            <div className="relative w-32 h-32 mx-auto">
              <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                ...
                <path
                  ...
                  strokeLinecap="round"
                  className="text-gray-900 dark:text-white transition-all duration-700"
                />
              </svg>
