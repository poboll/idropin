-- 创建任务更多信息表
CREATE TABLE IF NOT EXISTS task_more_info (
...
COMMENT ON COLUMN task_more_info.task_id IS '任务ID';
COMMENT ON COLUMN task_more_info.ddl IS '截止时间提示';
COMMENT ON COLUMN task_more_info.tip IS '任务提示信息';
COMMENT ON COLUMN task_more_info.info IS '任务详细信息';
...
COMMENT ON COLUMN task_more_info.bind_field IS '绑定字段';
