...
public interface SystemConfigMapper extends BaseMapper<SystemConfig> {
    ...
    @Select("SELECT * FROM sys_system_config WHERE is_enabled = true ORDER BY config_key")
    List<SystemConfig> findAllEnabled();

    @Select("SELECT * FROM sys_system_config ORDER BY config_type, config_key")
    List<SystemConfig> findAll();

    @Select("SELECT * FROM sys_system_config WHERE config_key = #{key}")
    SystemConfig findByKey(@Param("key") String key);

    @Select("SELECT * FROM sys_system_config WHERE config_type = #{type} ORDER BY config_key")
